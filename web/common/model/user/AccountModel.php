...
class AccountModel extends \web\common\model\Model {
    ...
    public function getList($pageIndex = -1, $pageSize = -1, $filter = '', $order = 'id asc') {
        ...
        $sql = 'select a.id, a.username,a.lxr,a.mobile,a.create_time,a.update_time,a.is_admin,a.is_hidden,b.name as role_name from ' . $this->getTableName() .' a left join '.$m->getTableName().' b on a.role_id=b.id';
        if (!empty($filter))
